...
def make_data():
    ...
    pw = ""
    department = random.randrange(1,45)
    return (sung + name, email, department, pw)
...
with conn:
    cur = conn.cursor()
    sql1 = "truncate table Doctors"
    sql = "insert into Doctors(name, email, departmentId, password) values(%s,%s,%s,%s) on duplicate key update email = email"
    cur.execute(sql1)
